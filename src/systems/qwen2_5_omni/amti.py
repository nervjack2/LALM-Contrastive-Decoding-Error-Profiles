...
class AMTISystem(Qwen2_5OmniSystem):
    def __init__(self, config):
        super().__init__(config)
        self.omega = self.model_config["amti"].get("omega", 1.2)
        self.tau = self.model_config["amti"].get("tau", 0.1)
        self.negative_prompt = self.model_config["amti"].get("negative_prompt", "Output Error")
    ...
    def inference(self, audios: list[np.ndarray], texts: list[str], ids: list[str], max_new_tokens: int = 512) -> str:
        ...
        amti_logits_processor = self.prepare_logits_processor(inputs.copy())
