...
class SAKURASequence(Dataset):
    def __init__(self, subject: str, multi: bool=False, reasoning: bool=False, llm_judge: bool=False, judge_mode: str="", prompt_mode: str=""):
    ...
    def task_description(self):
        if self.reasoning:
            ...
            prompt = "Answer the multiple-choice question. Answer with 'x', where x is the right letter in lowercase. Only output the letter of your answer."
            if self.llm_judge:
                print("LLM as a judge must be used with reasoning mode (-r).")
                exit(0)
        return prompt
    ...
    def __getitem__(self, idx):
        ...
        if not self.llm_judge or self.prompt_mode != "regular":
            inst["text_input"] = self.task_description + "\n\n" + full_prompt
        else:
